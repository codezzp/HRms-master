		...
		<div id="menu">
			...
			<p>
				<input type="button" id="query" value="查询 >" class="menu_button"
					...
					onclick="javascript:window.location.href='system_query.jsp'">
				<input type="button" id="delete" value="删除" class="menu_button"
					onmousedown="changeBC('delete')" onmouseup="originalBC('delete')"
					onclick="javascript:window.location.href='system_delete.jsp'">
		</div>
		...
		<div id="content">
			<form action="">
				<table width="100%" border="1px solid #FFFFFF"
					...
					<tr style="background-color: rgb(55, 133, 147); color: #F0F0F0">
						<td width="12%">管理员编号</td>
						<td width="12%">管理员昵称</td>
						<td width="13%">账号</td>
						<td width="13%">密码</td>
						<td width="13%">管理员邮箱</td>
						<td width="12%">管理员真实姓名</td>
						<td width="13%">管理员身份证号</td>
					...
					<%
						...
						if (db1.createConn()) {
							...
							while (db1.next()) {
								out.println("<tr>");
								out.println("<td>" + db1.getValue("id") + "</td>");
								out.println("<td>" + db1.getValue("nickname") + "</td>");
								out.println("<td>" + db1.getValue("account") + "</td>");
								out.println("<td>" + db1.getValue("password") + "</td>");
								out.println("<td>" + db1.getValue("email") + "</td>");
								out.println("<td>" + db1.getValue("actualname") + "</td>");
								out.println("<td>" + db1.getValue("identatyid") + "</td>");
								out.println("<td>" + db1.getValue("time") + "</td>");
								out.println("</tr>");
							}
							db1.closeRs();
							db1.closeStmt();
